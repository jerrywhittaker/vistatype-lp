...
def test_short_cut_key_hover_text_matches_the_label(repo_root):
    """Jerry, 9/23/2026: the label on 348 was copied from 357 and kept its hover text,
    "Shift+Alt+Ctrl+_". The hover text is set to the label's own caption when the box opens.
    (356 had the same label for one build, 3.0.500, and was put back to its 9/17/2026 version.)"""
    for form in ("Lp_Horz_To_Vert_List_Form",):
        code = "\n".join(form_code(repo_root, form))
